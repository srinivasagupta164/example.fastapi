from sqlalchemy import create_engine
from sqlalchemy.ext.declarative import declarative_base
from sqlalchemy.orm import sessionmaker
import psycopg2
from psycopg2. extras import RealDictCursor
import time
from . Config import settings

#SQLALCHENY_DATABASE_URL = "postgresql://<username>:<password>@<ip-address/hostname>/<databse_name>"
SQLALCHENY_DATABASE_URL = f"postgresql://{settings.database_username}:{settings.database_password}@{settings.database_hostname}:{settings.database_port}/{settings.database_name}"

engine = create_engine(SQLALCHENY_DATABASE_URL)

sessionLocal =sessionmaker(autocommit=False, autoflush=False, bind=engine)

Base =declarative_base()

# Dependency
def get_db():
    db = sessionLocal() 
    try:
        yield db
    finally:
        db.close()



#while True:
    #try:
        #conn = psycopg2.connect(host="Localhost",database="fastapi",user="postgres",password ="Jayandra98",cursor_factory=RealDictCursor)
        #cursor = conn.cursor()
        #print("database connected succesfully")
        #break
    #except Exception as error:
        #print("connection to database is failed")
        #print("Error",error)
        #time.sleep(2)
